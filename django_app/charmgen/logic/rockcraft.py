...
class RockcraftGenerator:
    ...
    def _run_command(self, command, status_callback=None):
        ...
        if return_code != 0:
            # The CalledProcessError will now contain the full output
            raise subprocess.CalledProcessError(return_code, command)

    def generate(self, status_callback=None):
        """Initializes and packs the Rock, with a callback for status updates."""
        
        if status_callback:
            status_callback("Initializing Rockcraft...")
        self._run_command(['rockcraft', 'init', '--profile=flask-framework'], status_callback)
